...
test.describe(() => {
    ...
    [
      ...
      [
        ...
        ({
          ...
          test(`should unarchive ${otherSelection.length + 1} searched selected ${otherSelection.length ? 'notes' : 'note'} with ${subsequentSelect} subsequent select`, async ({
            page,
            tapOrClick,
            activeNotesPageFxt,
          }) => {
            ...
            for (const { title, content } of finalActivePage) {
      ...
      [
        ...
        test(`should undo after unarchiving ${otherSelection.length + 1} searched selected ${otherSelection.length ? 'notes' : 'note'} with ${subsequentSelect} subsequent select`, async ({
          page,
          tapOrClick,
          activeNotesPageFxt,
        }) => {
  ...
  [
    ...
    test(`should unarchive selected archived notes with ${subsequentSelect} subsequent select`, async ({
      page,
      tapOrClick,
      activeNotesPageFxt,
      archiveNotesPageFxt,
      ...
      await expect(page.getByText(/^2 notes unarchive$/)).toBeVisible();
    ...
    test(`should undo after unarchiving selected archived notes with ${subsequentSelect} subsequent select`, async ({
      page,
      tapOrClick,
      activeNotesPageFxt,
      archiveNotesPageFxt,
