...
test.describe(() => {
  ...
  test.describe('Unarchives After Searching Notes', () => {
    ...
    [
      ...
      ({ otherSelection, toastMessage, finalArchivePage, finalActivePage }) => {
        test(`should unarchive ${otherSelection.length + 1} searched selected ${otherSelection.length ? 'notes' : 'note'}`, async ({
          page,
          tapOrClick,
          activeNotesPageFxt,
        }) => {
          ...
          await page.getByText(/^Note Title 3$/).click({
            delay: 750,
          });
    ...
    [
      ...
      test(`should undo after unarchiving ${otherSelection.length + 1} searched selected ${otherSelection.length ? 'notes' : 'note'}`, async ({
        page,
        tapOrClick,
        activeNotesPageFxt,
      }) => {
  ...
  test('should unarchive selected archived notes', async ({
    page,
    tapOrClick,
    activeNotesPageFxt,
    archiveNotesPageFxt,
    ...
    await page.getByText(/^Note Title 4$/).click({
      delay: 750,
    });
  ...
  test('should undo after unarchiving selected archived notes', async ({
    page,
    tapOrClick,
    activeNotesPageFxt,
    archiveNotesPageFxt,
